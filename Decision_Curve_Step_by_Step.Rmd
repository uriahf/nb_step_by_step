...
library(metathis)
meta() %>%
  meta_name("github-repo" = "uriahf/nb_step_by_step") %>%
  meta_social(
    title = "Decision Curve - Step by Step",
    description = "This presentation follow the article `A simple, step-by-step  guide to interpreting decision curve analysis`",
    url = "https://nb-step-by-step.netlify.app/decision_curve_step_by_step",
    image = "https://nb-step-by-step.netlify.app/share-card.png",
    og_type = "website",
...
- All interactive plots in this presentation were created with **[rtichoke](https://uriahf.github.io/rtichoke/)** R package (I am the author 👋), you are also invited to explore **.rtichoke_blog[[rtichoke blog](https://rtichoke-blog.netlify.app/)]** for reproducible examples and some theory.
...
#### Select patients for biopsy amongst men with elevated PSA
...
create_calibration_curve(
  ...
  reals = list(scenario1$event),
  type = "smooth",
  histogram_included = FALSE, size = 500
) %>% 
  plotly::partial_bundle()
...
prepare_performance_data(
  ...
  dplyr::filter(probability_threshold <= 0.2) %>% 
  plot_decision_curve(
  size = 500
) %>%
  plotly::animation_slider(
  ...
  plotly::layout(
    xaxis = list(
      range = list(-0.01, 0.205)
    ),
    yaxis = list(
      range = list(-0.01, 0.25)
    )
  )
...
# Step 1 
# 👣: Benefit is **.green[Good]**

---

### Step 1 👣: Benefit is **.green[Good]**
...
“missing an infected patiant is 4 times worse than giving antibiotics to a healthy patient."
...
dc_performance_data <- rtichoke::prepare_performance_data(
  ...
  dplyr::mutate(
    slider_text = 
      glue::glue("Probability Threshold: {stringr::str_pad(probability_threshold, 8, 'right')} \\
...
I will be indifferent 😐 for having 1 **.green[TP]** for 4 **.red[FP]**
$$p_t = \frac{1}{1 + 4} = 0.2$$
$$\frac{p_t}{1 - p_t} = \frac{0.2}{1 - 0.2} = \frac{1}{4}$$
...
I will be sad 🙁 for having 1 **.green[TP]** for 5 **.red[FP]**
$$p_t = \frac{1}{1 + 4} = 0.2$$
$$\frac{p_t}{1 - p_t} = \frac{0.2}{1 - 0.2} = \frac{1}{4}$$
In that case the Net Benefit of using the model will be:
...
I will be happy 🙂 for having 1 **.green[TP]** for 3 **.red[FP]**
$$p_t = \frac{1}{1 + 4} = 0.2$$
$$\frac{p_t}{1 - p_t} = \frac{0.2}{1 - 0.2} = \frac{1}{4}$$
In that case the Net Benefit of using the model will be:
...
prevalence_for_nb_reference_line <- rtichoke:::get_prevalence_from_performance_data(
    performance_data_dc) %>% 
  unique()
...
conventional <- plotly::plot_ly(
  ...
  "decision",
  plotly = TRUE,
  prevalence_for_nb_reference_line,
